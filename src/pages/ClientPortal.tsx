...
const STATUS_STEPS = [
  ...
  {
    key: 'installe',
    label: { fr: 'Installation', ar: 'التركيب', tn: 'التركيب', en: 'Installation', it: 'Installazione' },
  ...
  {
    key: 'livree',
    label: { fr: 'TERMINÉ ✓', ar: 'مكتمل ✓', tn: 'مكتمل ✓', en: 'TERMINATED ✓', it: 'COMPLETATO ✓' },
...
const content = {
  ...
  placeholder: { fr: 'Collez votre code de commande', ar: 'أدخل رمز التتبع الخاص بك', tn: 'دخل كود التتبع متاعك', en: 'Enter your tracking code (e.g: A1B2C3D4E)', it: 'Inserisci il tuo codice di tracciamento' },
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '20px' }}>
                  ...
                  <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 2px 12px rgba(0,0,0,0.05)', border: '1px solid #E2E8F0', textAlign: isRTL ? 'right' : 'left', overflow: 'hidden' }}>
                    <div style={{ background: 'linear-gradient(135deg, #1D3E61, #1A5DA8)', color: 'white', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <User size={18} />
                      <h3 style={{ fontFamily: 'Rajdhani, sans-serif', fontWeight: 600, fontSize: '13px', letterSpacing: '0.5px', textTransform: 'uppercase', margin: 0 }}>{loc(content.client_info)}</h3>
                    </div>
                  ...
                  <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 2px 12px rgba(0,0,0,0.05)', border: '1px solid #E2E8F0', textAlign: isRTL ? 'right' : 'left', overflow: 'hidden' }}>
                    <div style={{ background: 'linear-gradient(135deg, #1D3E61, #1A5DA8)', color: 'white', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <ShoppingBag size={18} />
                      <h3 style={{ fontFamily: 'Rajdhani, sans-serif', fontWeight: 600, fontSize: '13px', letterSpacing: '0.5px', textTransform: 'uppercase', margin: 0 }}>{loc(content.order_detail)}</h3>
                    </div>
                    <div style={{ padding: '24px' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {order.items.map((item, i) => (
                          <div key={i} style={{ padding: '12px', background: 'linear-gradient(135deg, #F0FDF4, #F5F7FA)', borderRadius: '10px', border: '1px solid #E2E8F0', borderLeft: '4px solid #81C063' }}>
                            <div style={{ fontFamily: 'Rajdhani, sans-serif', fontWeight: 700, fontSize: '15px', color: '#1A5DA8', letterSpacing: '0.5px', marginBottom: '4px' }}>
                              {item.productName}
                            </div>
                      ...
                      <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #E2E8F0' }}>
                        {(() => {
                          ...
                          return (
                            <>
                              {order.totalSurcharge && order.totalSurcharge > 0 ? (
                                <>
                                  <div style={{ display: 'flex', flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', fontSize: '13px', color: '#6B7280', fontFamily: 'DM Sans, sans-serif', marginBottom: '4px' }}>
                                    <span>Base HT (hors couleur)</span><span>{formatDT(order.baseBrutHT || 0)}</span>
                                  </div>
                                  <div style={{ display: 'flex', flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', fontSize: '13px', color: '#6B7280', fontFamily: 'DM Sans, sans-serif', marginBottom: '4px' }}>
                                    <span>Supplément couleur</span><span>+{formatDT(order.totalSurcharge)}</span>
                                  </div>
                                </>
                              ) : null}

                              <div style={{ display: 'flex', flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', fontSize: '14px', color: '#0F172A', fontFamily: 'DM Sans, sans-serif', fontWeight: 700, marginBottom: '4px' }}>
                                <span>Total brut HT</span><span>{formatDT(order.brutHT || 0)}</span>
                              </div>

                              {order.remise > 0 && (
                                <div style={{ display: 'flex', flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', fontSize: '13px', color: '#EF4444', fontFamily: 'DM Sans, sans-serif', fontWeight: 500, marginBottom: '4px' }}>
                                  <span>Remise ({remisePct}%)</span><span>-{formatDT(order.remise)}</span>
                                </div>
                              )}

                              <div style={{ display: 'flex', flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', fontSize: '14px', color: '#0F172A', fontFamily: 'DM Sans, sans-serif', fontWeight: 700, marginBottom: '8px', paddingBottom: '8px', borderBottom: '1px dashed #E2E8F0' }}>
                                <span>Total net HT</span><span>{formatDT(order.netHT)}</span>
                              </div>
                              ...
                              <div style={{ display: 'flex', flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', fontSize: '13px', color: '#6B7280', fontFamily: 'DM Sans, sans-serif', marginBottom: '4px' }}>
                                <span>Base TVA</span><span>{formatDT(order.baseTVA || 0)}</span>
                              </div>
